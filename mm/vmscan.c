...
#ifdef CONFIG_THROTTLE_BG
	/*
	 * 1. sigle zone will come in frequently, so we thinks foreground task
	 * no need wait and should to reclaim page return fast.
	 * 2. We add this condition behind of count_vm_event(PGSCAN_DIRECT_THROTTLE)
	 * is for lmkd kill some task this moment.
	 */
	group = rsc_task_group(current);
	if (is_foreground(group)
		|| (current_uid().val == 0) || (current_uid().val == 1000))
		goto out;
...
#ifdef CONFIG_THROTTLE_BG
	c2 = local_clock();
	pr_info("throttle group %d task <%s:%d><%s:%d> %lu us do direct reclaim",
		group, current->comm, current->pid,
		current->group_leader ? current->group_leader->comm : "unknown",
		current->group_leader ? current->group_leader->pid : 9999,
		(c2 - c1) / 1000);
#endif
