...
static void gc_temperary_work_fn(struct work_struct *work)
{
	struct f2fs_sb_info *sbi = container_of(work, struct f2fs_sb_info, gc_temperary_work.work);
...
out:
	...
	if (!strcmp(a->attr.name, "gc_urgent")) {
		if (t == 0) {
			sbi->gc_mode = GC_NORMAL;
#ifdef CONFIG_FS_MODIFY
			cancel_delayed_work_sync(&sbi->gc_temperary_work);
#endif
		} else if (t == 1) {
#ifdef CONFIG_FS_MODIFY
			cancel_delayed_work_sync(&sbi->gc_temperary_work);
			...
			work_status = work_busy(&(sbi->gc_temperary_work.work));
			if (!work_status) {
				sbi->gc_mode = GC_URGENT_HIGH;
				if (sbi->gc_thread) {
					sbi->gc_thread->gc_wake = 1;
			...
			mod_delayed_work(system_wq, &sbi->gc_temperary_work, msecs_to_jiffies(10000));
#else
		} else if (t == 3) {
...
#ifdef CONFIG_FS_MODIFY
	INIT_DELAYED_WORK(&sbi->gc_temperary_work, gc_temperary_work_fn);
#endif
