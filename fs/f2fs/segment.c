...
static struct discard_policy dpolicys[MAX_DPOLICY] = {
	...
	{DPOLICY_BG,		DISCARD_GRAN_BG,	false,		MAX_PLIST_NUM,		true,	false,		/* >= 2M */
		/* max_req	interval(ms)	min_interval	mid_interval			max_interval  time_out*/
/* 2M.. */	{{UINT_MAX,	100},
/* 64K..2M */	{0,		0},
/* 4K..64K */	{0,		0}},		0,		DEF_MID_DISCARD_ISSUE_TIME,	DEF_MAX_DISCARD_ISSUE_TIME,	false},
	{DPOLICY_BL,		DISCARD_GRAN_BL,	true,		MAX_PLIST_NUM - 1,	true,	false,	/* >=64K */
		{{1,		50},
		{2,		100},
		{0,		0}},		0,		DEF_MID_DISCARD_ISSUE_TIME,	DEF_MAX_DISCARD_ISSUE_TIME,	false},
	{DPOLICY_FORCE,		DISCARD_GRAN_FORCE,	true,		MAX_PLIST_NUM - 1,	true,	false,	/* >= 4K */
		{{1,		50},
		{2,		100},
		{4,		2000}},		0,		DEF_MID_DISCARD_ISSUE_TIME,	DEF_MAX_DISCARD_ISSUE_TIME,	false},
	{DPOLICY_FSTRIM,	DISCARD_GRAN_FORCE,	false,		MAX_PLIST_NUM,		true,	false,	/* >= 4K */
		{{8,		50},
		{8,		50},
		{8,		50}},		0,		DEF_MID_DISCARD_ISSUE_TIME,	DEF_MAX_DISCARD_ISSUE_TIME,	false},
	{DPOLICY_UMOUNT,	DISCARD_GRAN_BG,	false,		MAX_PLIST_NUM,		true,	false,	/* >= 2M */
		{{UINT_MAX,	50},
		{0,		50},
		{0,		50}},		0,		DEF_MID_DISCARD_ISSUE_TIME,	DEF_MAX_DISCARD_ISSUE_TIME,	true}
};
#endif
...
#ifdef CONFIG_FS_MODIFY
bool f2fs_need_SSR_custom(struct f2fs_sb_info *sbi, int type, int blocks)
{
	...
	u64 left_space = (total_blocks - valid_blocks) << 2; //KB
	...
	if (unlikely(is_sbi_flag_set(sbi, SBI_CP_DISABLED)))
		return true;
	if (blocks >= 256 && type == CURSEG_WARM_DATA &&
		free_sections(sbi) > dirty_sum + 3 * reserved_sections(sbi) / 2)
		return false;
	if(free_sections(sbi) <= (dirty_sum + 2 * reserved_sections(sbi)))
		return true;
	if(blocks > 32)
		return false;

	left_space -= ovp_segments * 2048;
	...
	if(left_space > spaces_limit) //8GB
		return false;

	return (free_segs - ovp_segments) * 100 / (left_space / 2048) <= waterline;
}
#endif
...
static unsigned int __issue_discard_cmd_orderly(struct f2fs_sb_info *sbi,
					struct discard_policy *dpolicy, int spolicy_index)
#else
static unsigned int __issue_discard_cmd_orderly(struct f2fs_sb_info *sbi,
					struct discard_policy *dpolicy)
#endif
...
#ifdef CONFIG_FS_MODIFY
		if (dpolicy->io_aware && !f2fs_gc_usb_screen_status(sbi)) {
			if (!is_idle(sbi, DISCARD_TIME)) {
				io_interrupted = true;
				break;
			}
		}
...
static int issue_discard_thread(void *data)
{
	...
	do {
		...
		if (sbi->gc_mode == GC_URGENT_HIGH ||
			...
			__init_discard_policy(sbi, &dpolicy, discard_type, 1);
		else
...
static void allocate_segment_by_default(struct f2fs_sb_info *sbi,
						int type, bool force, int nr_blocks)
#else
static void allocate_segment_by_default(struct f2fs_sb_info *sbi,
						int type, bool force)
#endif
{
	...
	if (force)
		...
		new_curseg(sbi, type, false);
#ifdef CONFIG_FS_MODIFY
	else if (f2fs_need_SSR_custom(sbi, type, nr_blocks) &&
#else
...
#ifdef CONFIG_FS_MODIFY
	SIT_I(sbi)->s_ops->allocate_segment(sbi, type, true, 1);
#else
...
void f2fs_allocate_data_block(struct f2fs_sb_info *sbi, struct page *page,
		...
		struct f2fs_io_info *fio, int nr_blocks)
#else
void f2fs_allocate_data_block(struct f2fs_sb_info *sbi, struct page *page,
		block_t old_blkaddr, block_t *new_blkaddr,
		struct f2fs_summary *sum, int type,
...
#ifdef CONFIG_FS_MODIFY
			sit_i->s_ops->allocate_segment(sbi, type, false, nr_blocks);
#else
