...
block_t f2fs_get_free_block_count(struct f2fs_sb_info *sbi)
{
...
static inline void f2fs_set_gc_control_info(struct f2fs_sb_info *sbi,
						int iolimit, bool is_greedy, bool enable_idle)
{
	sbi->gc_control_info.iolimit = 0;
	sbi->gc_control_info.is_greedy = is_greedy;
	sbi->gc_control_info.idle_ignore = enable_idle;
}

static unsigned int f2fs_get_gc_policy(struct f2fs_sb_info *sbi, int *level_ret)
{
	unsigned int wait_ms;
	unsigned int free_secs = free_sections(sbi);
	/* # of free blocks in main area, include op area */
	block_t free_block_count = f2fs_get_free_block_count(sbi);
	int *level = sbi->gc_stat.level;

	if (free_secs > level[BG_GC_LEVEL1]) {
		wait_ms = MIN_WT * 120;
		f2fs_set_gc_control_info(sbi, 0, false, false);
		*level_ret = BG_GC_LEVEL1;
	} else if (free_secs > level[BG_GC_LEVEL2]) {
		if (f2fs_disk_is_frag(sbi, free_block_count, free_secs)) {
			wait_ms = MIN_WT * 5;
			f2fs_set_gc_control_info(sbi, 0, false, false);
		} else {
			wait_ms = MIN_WT * 60;
		...
		*level_ret = BG_GC_LEVEL3;
		...
		if (f2fs_disk_is_frag(sbi, free_block_count, free_secs)) {
			wait_ms = DEF_GC_USB_SCREEN_SLEEP_MIN_TIME;
			f2fs_set_gc_control_info(sbi,
		...
		*level_ret = BG_GC_LEVEL4;
		...
		wait_ms = MIN_WT * 10;
		f2fs_set_gc_control_info(sbi, 0, true, true);
...
#ifdef CONFIG_FS_MODIFY
		wait_ms = f2fs_get_gc_policy(sbi, &level);
#endif
...
#ifdef CONFIG_FS_MODIFY
		if (f2fs_gc_usb_screen_status(sbi) || sbi->gc_control_info.idle_ignore) {
...
next:
		...
		if (print_time >= DEF_GC_THREAD_PRINT_TIME) {
			pr_info("%s lcw usb: %d, lcd: %d, gc_mode: %d, user: %u, dirty: %u, "
				" invalid: %u, free: %u, reclaimable: %u, used: %d%%, undiscard: %u\n",
				__func__, usb_state(sbi), screen_state(sbi),
				sbi->gc_mode, sbi->user_block_count >> sbi->log_blocks_per_seg, dirty_segments(sbi),
				(sbi->user_block_count - written_block_count(sbi) - free_user_blocks(sbi)) >> sbi->log_blocks_per_seg,
				free_segments(sbi) < overprovision_segments(sbi) ? 0 : free_segments(sbi) - overprovision_segments(sbi),
				(sbi->user_block_count - written_block_count(sbi)) >> sbi->log_blocks_per_seg,
				utilization(sbi), dcc->undiscard_blks);
			print_time = 0;
		}

		if (sbi->gc_control_info.loop_cnt > GC_THREAD_LOOP_SCHEDULE) {
			msleep(MIN_WT);
 ...
 */
static void f2fs_adjust_gc_levels(struct f2fs_sb_info *sbi)
{
	int *level = sbi->gc_stat.level;
	int extra;
	...
	level[BG_GC_LEVEL5] = level[BG_GC_LEVEL6] +
		get_level_interval(sbi, 256, MIN_SECS, 1);
	...
	level[BG_GC_LEVEL3] = level[BG_GC_LEVEL4] +
		get_level_interval(sbi, MIN_SEGS, MIN_SECS, 1);
	if ((overprovision_sections(sbi) > level[BG_GC_LEVEL3]) &&
			(overprovision_sections(sbi) - level[BG_GC_LEVEL3]) *
			sbi->segs_per_sec > EXTRA_SEGS_IN_OVP) {
		extra = (overprovision_sections(sbi) - level[BG_GC_LEVEL3]) / 4;
...
static int f2fs_init_gc_levels(struct f2fs_sb_info *sbi)
{
	f2fs_adjust_gc_levels(sbi);
	return 0;
}

int f2fs_build_gc_control_info(struct f2fs_sb_info *sbi)
{
	int err = 0;
	...
	sbi->gc_control_info.idle_ignore = false;

	err = f2fs_init_gc_levels(sbi);
