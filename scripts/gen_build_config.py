...
def main(**args):
    ...
    if (build_mode == 'eng') or (build_mode == 'userdebug'):
        ...
        if oversea == 'yes':
            abs_mode_config = "%s/kernel/configs/%s"%(abs_kernel_dir, mode_config)
            modules_list = []
            with open(abs_mode_config, 'r') as fh:
                datas = fh.readlines()
                m_pattern = re.compile('=\s*m')
                for each in datas:
                    if m_pattern.search(each):
                        modules_list.append(each)
            if modules_list:
                mode_module_config = "%s/%s/%s_modules.config"%(abs_kernel_dir, os.path.dirname(out_file), build_mode)
                if not os.path.exists(os.path.dirname(mode_module_config)):
                    os.makedirs(os.path.dirname(mode_module_config))
                with open(mode_module_config, 'w') as fh:
                    for each in modules_list:
                        fh.write(each)
                mode_module_config = '../../../${REL_KERNEL_PATH}/${OUT_DIR}/%s_modules.config'%build_mode
            mode_config = '../../../${REL_KERNEL_PATH}/${OUT_DIR}/gki_kernel/common/.config'
            mode_first = True
    ...
    file_text.append("GKI_PATH=${ROOT_DIR}/../prebuilts/perl/linux-x86/bin:${ROOT_DIR}/build/build-tools/path/linux-x86:/usr/bin:/bin")
    ...
    if not special_defconfig:
        ...
        if mode_first:
            all_defconfig = '%s %s ../../../${REL_KERNEL_PATH}/${OUT_DIR}/%s.config %s %s' % (special_defconfig, mode_config, project, kernel_defconfig_overlays, mode_module_config)
        pre_oversea_defconfig_cmd = ''
        if oversea == 'yes':
            pre_oversea_defconfig_cmd = '&& if [ ! -d ${OUT_DIR}/gki_kernel/common/ ];then mkdir -p ${OUT_DIR}/gki_kernel/common;fi && touch ${OUT_DIR}/gki_kernel/common/.config '
        pre_defconfig_cmds = 'PRE_DEFCONFIG_CMDS=\"cp -p ${KERNEL_DIR}/%s/%s ${OUT_DIR}/%s.config %s\"' % (defconfig_dir, project_defconfig_name, project, pre_oversea_defconfig_cmd)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--oversea", dest="oversea", help="BBK_PRODUCT_OVERSEAS.", default="no")
