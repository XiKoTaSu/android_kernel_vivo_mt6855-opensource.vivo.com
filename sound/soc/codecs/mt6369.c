...
static void mtk_hp_enable(struct mt6369_priv *priv)
{
	...
	printk("%s:regmap_write:MT6369_AUDDEC_ANA_CON4=0x33\n",__func__);
...
static int mtk_hp_impedance_enable(struct mt6369_priv *priv)
{
	...
	printk("%s:regmap_update_bits:MT6369_AUDDEC_ANA_CON4:(0x7 << 4)=0x0\n",__func__);
	regmap_update_bits(priv->regmap, MT6369_AUDDEC_ANA_CON4,
			   RG_HPLOUTPUTSTBENH_VAUDP15_MASK_SFT, 0x0);
	printk("%s:regmap_update_bits:MT6369_AUDDEC_ANA_CON4:(0x7 << 0)=0x0\n",__func__);
	/* Disable HP aux CMFB loop */
...
static int mtk_hp_impedance_disable(struct mt6369_priv *priv)
{
	...
	printk("%s:regmap_update_bits:MT6369_AUDDEC_ANA_CON4:((0x7 << 4)=(0x3 << 4))\n",__func__);
	regmap_update_bits(priv->regmap, MT6369_AUDDEC_ANA_CON4,
			   RG_HPLOUTPUTSTBENH_VAUDP15_MASK_SFT,
			   0x3 << RG_HPLOUTPUTSTBENH_VAUDP15_SFT);
	printk("%s:regmap_update_bits:MT6369_AUDDEC_ANA_CON4:((0x7 << 0)=(0x3 << 0))\n",__func__);
...
static struct bin_attribute codec_dev_attr_reg_dump = {
	.attr = {
		.name = "reg",
...
static int mt6369_codec_probe(struct snd_soc_component *cmpnt)
{
	...
	int ret = 0;
	struct kobject *kobj = &cmpnt->dev->kobj;
	char name[64] = "audio-mtksocanaaudio";
	...
	/* add by duanyitao for dump-reg node start */
	codec_dev_attr_reg_dump.private = priv;
	kobj = kobject_create_and_add(name, kernel_kobj);
	ret = sysfs_create_bin_file(kobj, &codec_dev_attr_reg_dump);
	if (ret)
		pr_info("sysfs_create_bin_file kobj fail\n");
	/* add by duanyitao for dump-reg node end */
...
{
	struct mt6369_priv *priv =  PDE_DATA(file_inode(file));
...
static ssize_t mt6369_proc_read(struct file *file, char __user *buf,
...
static ssize_t mt6369_proc_write(struct file *f, const char __user *buf,
				    size_t count, loff_t *offset)
{
	struct mt6369_priv *priv =  PDE_DATA(file_inode(f));
	char input[MAX_DEBUG_WRITE_INPUT];
	char *temp = NULL;
	char *command = NULL;
	char *str_begin = NULL;
...
static const struct proc_ops mt6369_proc_debugfs_ops = {
	.proc_write = mt6369_proc_write,
	.proc_read = mt6369_proc_read,
};
...
static void __init mt6369_proc_dump_register(struct mt6369_priv *priv)
{
	...
	entry = proc_create_data("mtksocanaaudio", 0664, NULL, &mt6369_proc_debugfs_ops, priv);
	if (!entry)
...
static int mt6369_platform_driver_probe(struct platform_device *pdev)
{
	...
	mt6369_proc_dump_register(priv);
	if (ret) {
		dev_warn(&pdev->dev,
