...
#include <linux/miscdevice.h> // add by vivo audio 
#include "mt6358-accdet.h"
#include "mt6358.h"

#if IS_ENABLED(CONFIG_MTK_TYPEC_ACCDET)
...
/*add by vivo audio start*/
bool accdet_probe_status;
EXPORT_SYMBOL(accdet_probe_status);
static struct miscdevice *g_misc_dev = NULL;
/*add by vivo audio end*/
 ...
 */
#define MICBIAS_DISABLE_TIMER (3 * HZ) //modify by audio yanghen
...
{
	pr_info("%s:enter,accdet->cur_eint_state =%d\n", __func__,accdet->cur_eint_state);
	if (accdet->cur_eint_state == EINT_PLUG_IN) {
		mutex_lock(&accdet->res_lock);
...
/*add by vivo audio start*/
#if IS_ENABLED(CONFIG_MTK_TYPEC_ACCDET)
static u32 accdet_get_auxadc_debounce()
{
	int tries = 9;
	u32 adc = 0, old_adc;
	u32 debounce = 100;
	...
	old_adc = accdet_get_auxadc();
	while (tries-- > 0) {
		usleep_range(5000, 5010);
		adc = accdet_get_auxadc();
		if (adc >= old_adc && adc - old_adc < debounce)
			break;
		if (adc <= old_adc && old_adc - adc < debounce)
			break;
		old_adc = adc;
	}
	pr_info("%s: adc return value %d mv after %d tries\n", __func__, adc, tries);
	return adc;
}
#endif
/*add by vivo audio end*/
static void accdet_work_callback(struct work_struct *work)
{
	...
	/*add by vivo audio start*/
#if IS_ENABLED(CONFIG_MTK_TYPEC_ACCDET)
	if (accdet->eint_sync_flag && (accdet->accdet_status == PLUG_OUT)) {
		adval_pre = accdet_get_auxadc_debounce();
		max20328_switch_mode_event(MAX_MIC_GND_SWAP);
		adval_pos = accdet_get_auxadc_debounce();
		pr_info("%s: before adcval=%d mv  after adcval=%d mv\n", __func__, adval_pre, adval_pos);
		if ((adval_pos < adval_pre) && ((adval_pre - adval_pos) > 300)) {
			...
			msleep(5);
			adval_pre = accdet_get_auxadc();
			pr_info("%s: adcval=%d mv for pure impedence  before\n", __func__, adval_pre);
			max20328_switch_mode_event(MAX_MIC_GND_SWAP);
			msleep(5);
			adval_pos = accdet_get_auxadc();
...
#if IS_ENABLED(CONFIG_SND_SOC_MT6366_ACCDET) && IS_ENABLED(CONFIG_MTK_TYPEC_ACCDET)
int accdet_enable_irq(bool enable)
{
#if 1
	int ret = 0, mode = 0;

	accdet->cur_eint_state = enable ? EINT_PLUG_IN : EINT_PLUG_OUT;
...
EXPORT_SYMBOL(accdet_enable_irq);
#endif
...
void accdet_irq_handle(void)
{
	...
	pr_info("%s: irq_status =%d,eintID=%d,acc_sts=%d,eint_sts=%d\n", __func__,irq_status,
			eintID,acc_sts,eint_sts);
...
static int accdet_get_dts_data(void)
{
	...
	if (ret)
		accdet_pmic = 0x0;
	//zhucancan add comment
	/* MIC_BIAS_1P7 = 0, MIC_BIAS_1P8, MIC_BIAS_1P9, MIC_BIAS_2P0,
	MIC_BIAS_2P1, MIC_BIAS_2P5, MIC_BIAS_2P6, MIC_BIAS_2P7,
	MIC_BIAS_2P8, MIC_BIAS_2P85 */
	...
	//zhucancan add comment
	/* mic mode setting
	HEADSET_MODE_1(1) ACC mode
	HEADSET_MODE_2(2) DCC mode Low cost mode without internal bias
	HEADSET_MODE_6(6) DCC mode Low cost mode with internal bias */
	ret = of_property_read_u32(node,
	...
	//zhucancan add comment
	/* key mode and key threashold setting
	0 -> ACCDET_THREE_KEY -> headset-three-key-threshold
	1 -> ACCDET_FOUR_KEY -> headset-four-key-threshold
	2 -> ACCDET_TRI_KEY_CDD -> headset-three-key-threshold-CDD */
	ret = of_property_read_u32(node,
...
/*add by vivo audio start*/
static int mt_accdet_open(struct inode *inode, struct file *file)
{
    pr_info("+%s() success\n", __func__);
    return 0;
}

static int mt_accdet_release(struct inode *inode, struct file *file)
{
    pr_info("+%s() success\n", __func__);
    return 0;
}
	...
	unsigned long arg)
{
	...
	case GET_BUTTON_STATUS:
 	...
 	case ACCDET_AT_IOCTL_GET_USBC_STATUS:
 		res = accdet->accdet_status;
 		break;
 	case ACCDET_AT_IOCTL_GET_USBC_MG_STATUS:
 		res = -1; /* wangkairjptb add for default value */
#if IS_ENABLED(CONFIG_MTK_TYPEC_ACCDET)
 		res = get_usbc_mg_status();
#endif
 		break;
 	case ACCDET_AT_IOCTL_GET_MIC_ADC_VOLTAGE:
 		res = accdet_get_auxadc();
 		break;
 	case ACCDET_AT_IOCTL_GET_TYPE:
 		res = accdet->cable_type;
 		break;
	default:
		break;
	}
 	ret = copy_to_user(result, &res, sizeof(int));
 	pr_info("+%s() cmd(0x%x), res =%d, leave!", __func__, cmd, res);
	/*modify by vivo audio end*/
	return 0;
}
static long mt_accdet_compat_ioctl(struct file *file, unsigned int cmd,
        unsigned long arg)
{
	return mt_accdet_unlocked_ioctl(file, cmd, arg);
}
static const struct file_operations accdet_fops = {
	...
	// modify by vivo audio
	.open  = mt_accdet_open,  
 	.release = mt_accdet_release,
	.compat_ioctl   = mt_accdet_compat_ioctl,
};
...
/*add by vivo audio start*/
static struct miscdevice accdet_device = {
...
static int __init accdet_soc_init(void)
{
	...
	if (ret)
		return -ENODEV;
	if(g_misc_dev==NULL){
		ret = misc_register(&accdet_device);
		if(ret){
			pr_err("+%s() accdet device register failed\n");
			return ret;
		}
		g_misc_dev = &accdet_device;
		pr_info("+%s() accdet device register success\n", __func__);
	}else{
	   pr_info("+%s() accdet device already register\n", __func__);	
	}
	return 0;
}
static void __exit accdet_soc_exit(void)
{
	...
	if (g_misc_dev != NULL) {
		...
		g_misc_dev = NULL;
		pr_info("+%s() deregister accdet device success\n", __func__);
