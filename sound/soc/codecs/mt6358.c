...
static int mt6358_amic_enable(struct mt6358_priv *priv)
{
	...
	if (mux_pga_l == PGA_MUX_AIN0 || mux_pga_l == PGA_MUX_AIN2 ||
		...
		regmap_update_bits(priv->regmap, MT6358_AUDENC_ANA_CON9,
				   0xff, 0x41); // modify MICBIAS0 2.1v by yanghen vivo audio
	}
	...
	if (mux_pga_l == PGA_MUX_AIN1 || mux_pga_r == PGA_MUX_AIN1) {
		...
		if (mic_type == MIC_TYPE_MUX_DCC_ECM_SINGLE)
			...
			regmap_write(priv->regmap,
				     MT6358_AUDENC_ANA_CON10, 0x0071); // modify MISBIAS1 = 2.7V by yanghen vivo audio
	}
...
static int mt6358_vow_amic_enable(struct mt6358_priv *priv)
{
	...
	if (mux_pga_l == PGA_MUX_AIN0) {
		...
		/* Enable MICBIAS0, MISBIAS0 = 1P9V */
		regmap_update_bits(priv->regmap, MT6358_AUDENC_ANA_CON9,
				   0xff, 0x45);  // modify MICBIAS0 2.1v by yanghen vivo audio
	}
	/* mic bias 1 */
	if (mux_pga_l == PGA_MUX_AIN1) {
		...
		if (mic_type == MIC_TYPE_MUX_DCC_ECM_SINGLE)
			...
			regmap_write(priv->regmap,
				     MT6358_AUDENC_ANA_CON10, 0x0071); // modify MICBIAS1 2.7v by yanghen vivo audio
	}
	/* mic bias 0 */
