...
static void send_key_event(u32 keycode, u32 flag)
{
	...
	case AS_KEY:
		...
		break;
	default:
		pr_info("accdet invalid key, ignore %d\n", flag);
		break;
	}
}
...
{
	pr_info("accdet %s+,accdet->cur_eint_state=%d\n", __func__,accdet->cur_eint_state);
	if (accdet->cur_eint_state == EINT_PLUG_IN) {
		/* disable vusb LP */
	...
	pr_info("accdet %s-,accdet->eint_sync_flag=%d\n", __func__,accdet->eint_sync_flag);
}
...
static inline void check_cable_type(void)
{
	...
	case PLUG_OUT:
		if (cur_AB == ACCDET_STATE_AB_00) {
			pr_info("%s : BSP.Audio PLUG_OUT ACCDET_STATE_AB_00 HEADSET_NO_MIC\n", __func__);
			mutex_lock(&accdet->res_lock);
			if (accdet->eint_sync_flag) {
			...
			pr_info("%s : BSP.Audio PLUG_OUT ACCDET_STATE_AB_01 HEADSET_MIC\n", __func__);
			mutex_lock(&accdet->res_lock);
			if (accdet->eint_sync_flag) {
			...
			pr_info("%s : BSP.Audio PLUG_OUT ACCDET_STATE_AB_11 NO_DEVICE\n", __func__);
	...
	case MIC_BIAS:
		if (cur_AB == ACCDET_STATE_AB_00) {
			pr_info("%s : BSP.Audio MIC_BIAS ACCDET_STATE_AB_00 multi_key_detection\n", __func__);
			mutex_lock(&accdet->res_lock);
			if (accdet->eint_sync_flag && (!(gpio_get_value(accdet->gpiopin)))) {
			...
			pr_info("%s : BSP.Audio MIC_BIAS ACCDET_STATE_AB_01 HEADSET_MIC\n", __func__);
			mutex_lock(&accdet->res_lock);
			if (accdet->eint_sync_flag) {
				accdet->accdet_status = MIC_BIAS;
				accdet->cable_type = HEADSET_MIC;
			...
			pr_info("%s : BSP.Audio MIC_BIAS ACCDET_STATE_AB_11 multi_key_detection\n", __func__);
	...
	case HOOK_SWITCH:
		if (cur_AB == ACCDET_STATE_AB_00)
		{
			pr_info("%s : BSP.Audio HOOK_SWITCH ACCDET_STATE_AB_00\n", __func__);
		}
		else if (cur_AB == ACCDET_STATE_AB_01)
		{
			pr_info("%s : BSP.Audio HOOK_SWITCH ACCDET_STATE_AB_01\n", __func__);
			mutex_lock(&accdet->res_lock);
			if (accdet->eint_sync_flag) {
				multi_key_detection(cur_AB);
				accdet->accdet_status = MIC_BIAS;
			...
			pr_info("%s : BSP.Audio HOOK_SWITCH ACCDET_STATE_AB_11 multi_key_detection\n", __func__);
			...
			struct snd_soc_card *card)
{
	...
	snd_jack_set_key(accdet->jack.jack, SND_JACK_BTN_3, KEY_IMUSIC_COLLECT); //duanyitao modified for imusic collect
