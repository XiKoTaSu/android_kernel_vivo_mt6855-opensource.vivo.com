...
/* vivo audio add for ana pa start */
#ifdef CONFIG_VIVO_ANA_PA
SND_SOC_DAILINK_DEFS(vivoplayback,
	DAILINK_COMP_ARRAY(COMP_CPU("ADDA")),
	DAILINK_COMP_ARRAY(COMP_CODEC("soc:odm:vivo-soc-codec@0",
				      "VIVO-Codec-dai")),
	DAILINK_COMP_ARRAY(COMP_EMPTY()));
#endif
/* vivo audio add for ana pa end */
...
/* vivo audio add for ana pa start */
#ifdef CONFIG_VIVO_ANA_PA
	{
		...
		SND_SOC_DAILINK_REG(vivoplayback),
	},
#endif
/* vivo audio add for ana pa end */
};
...
static int mt6885_mt6359p_dev_probe(struct platform_device *pdev)
{
	...
	if (!spk_node) {
#ifdef CONFIG_VIVO_ANA_PA /* vivo audio add for ana pa */
		dev_info(&pdev->dev, "ana pa, do not need spk_node\n");
#else
		dev_err(&pdev->dev,
