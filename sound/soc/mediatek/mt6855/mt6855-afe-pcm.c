...
#if IS_ENABLED(CONFIG_SND_SOC_VIVO_KTV)
#include "../../vivo/vivo_audio_ktv/vivo-audio-ktv.h"
#endif
				      ...
				      snd_pcm_uframes_t frames)
{
	...
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(component);
	struct snd_soc_dai *cpu_dai = asoc_rtd_to_cpu(rtd, 0);
	int cpu_dai_id = cpu_dai->id;
	struct mtk_base_afe_memif *memif = &afe->memif[cpu_dai_id];
	int irq_id = memif->irq_usage;
	struct mtk_base_afe_irq *irqs = &afe->irqs[irq_id];
	const struct mtk_base_irq_data *irq_data = irqs->irq_data;
	...
	if (cpu_dai_id == MT6855_MEMIF_DL1 || cpu_dai_id == MT6855_MEMIF_VUL12) {
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (cpu_dai_id == MT6855_MEMIF_DL1) {
		...
		if (cpu_dai_id == MT6855_MEMIF_VUL12) {
			vivo_audio_ktv_set_channels(substream->runtime->channels);
...
struct bin_attribute bin_attr_afe_dump_reg = {
	.attr = {
		.name = "reg",
		.mode = 0444,
	},
...
static int mt6855_afe_component_probe(struct snd_soc_component *component)
{
	...
	int ret = 0;
	struct kobject *kobj = &component->dev->kobj;
	char name[64] = "audio-mtksocaudio";
	...
	if (component) {
		...
		bin_attr_afe_dump_reg.private = (void *)afe;
		kobj = kobject_create_and_add(name, kernel_kobj);
		ret = sysfs_create_bin_file(kobj, &bin_attr_afe_dump_reg);
		if (ret)
...
static ssize_t mtk_afe_proc_write(struct file *f, const char __user *buf,
			      size_t count, loff_t *offset)
{
	struct mtk_base_afe *afe = PDE_DATA(file_inode(f));
	char input[MAX_DEBUG_WRITE_INPUT];
	char *temp = NULL;
	char *command = NULL;
	char *str_begin = NULL;
...
static ssize_t mt6855_proc_read(struct file *file, char __user *buf,
				   size_t count, loff_t *pos)
{
	struct mtk_base_afe *afe = PDE_DATA(file_inode(file));
	const int size = AFE_SYS_DEBUG_SIZE;
	char *buffer = NULL; /* for reduce kernel stack */
	int n = 0, ret = 0;
...
static const struct proc_ops mt6855_proc_debugfs_ops = {
	.proc_write = mtk_afe_proc_write,
	.proc_read = mt6855_proc_read,
};

static void __init mt6855_proc_dump_register(struct mtk_base_afe *afe)
{
	...
	entry = proc_create_data("mtksocaudio", 0664, NULL, &mt6855_proc_debugfs_ops, afe);
	if (!entry)
