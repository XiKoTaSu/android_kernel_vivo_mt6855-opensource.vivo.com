...
struct mtk_base_afe *afe_base;  // add by vivo audio yanghen for i2s clock ctrl
EXPORT_SYMBOL(afe_base);
...
static int mt6789_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	afe->dev = &pdev->dev;
	dev = afe->dev;
	afe_base = afe; // add by vivo audio yanghen for i2s clock ctrl
