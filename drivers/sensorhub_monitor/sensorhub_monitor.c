...
extern void sensor_ready_notifier_chain_register(struct notifier_block *nb);
extern void sensor_ready_notifier_chain_unregister(struct notifier_block *nb);

static int sensorhub_callback(struct notifier_block *nb, unsigned long value, void *priv)
{
	char *envp[2];
	envp[1] = NULL;
...
static struct notifier_block sensorhub_notifier_block = {
	.notifier_call = sensorhub_callback,
	.priority = 0,
};

static int __init sensorhub_monitor_init(void)
{
	int ret = -1;
	...
	sensor_ready_notifier_chain_register(&sensorhub_notifier_block);
...
static void __exit sensorhub_monitor_exit(void)
{
	sensor_ready_notifier_chain_unregister(&sensorhub_notifier_block);
	device_destroy(sensorhub_class, devno);
	class_destroy(sensorhub_class);
	unregister_chrdev_region(devno, devicecount);
}

module_init(sensorhub_monitor_init);
module_exit(sensorhub_monitor_exit);

MODULE_AUTHOR("liu.yu@vivo.com");
