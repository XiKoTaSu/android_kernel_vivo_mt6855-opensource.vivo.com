...
static int timesync_comm_with_android_time(void)
{
	int ret = 0;
	struct rtc_time rt;
	struct timespec64 ts;
	struct sensor_comm_ctrl *ctrl = NULL;
	struct sensor_comm_android_time *time = NULL;
	...
	if (!ctrl)
		return -ENOMEM;
	ctrl->sensor_type = 0;
	ctrl->command = SENS_COMM_CRTL_ANDROID_TIME_SYNC_CMD;
	ctrl->length = sizeof(*time);
	time = (struct sensor_comm_android_time *)ctrl->data;

	ktime_get_real_ts64(&ts);
	rtc_time64_to_tm((ts.tv_sec - sys_tz.tz_minuteswest * 60), &rt);

	time->mon = rt.tm_mon + 1;
	time->day = rt.tm_mday;
	time->hour = rt.tm_hour;
	time->min = rt.tm_min;
	time->sec = rt.tm_sec;
	time->msec = ts.tv_nsec / 1000000;
	pr_info("host sync ap %02d-%02d %02d:%02d:%02d.%03d\n", time->mon,
															time->day,
															time->hour,
															time->min,
															time->sec,
...
static int timesync_comm_with(void)
{
	...
	timesync_comm_with_android_time();
