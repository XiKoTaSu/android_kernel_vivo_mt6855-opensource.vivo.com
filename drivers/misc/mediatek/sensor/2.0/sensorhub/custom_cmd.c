...
static int custom_cmd_slow_seq(int sensor_type, struct share_mem_cmd *shm_cmd)
{
	...
	if (!timeout) {
		pr_err("%s timeout! sensor_type:%d command:%d\n", __func__, sensor_type, shm_cmd->command);
		return -ETIMEDOUT;
	}
...
static int custom_cmd_fast_seq(int sensor_type, struct custom_cmd *cust_cmd)
{
	...
	timeout = wait_for_completion_timeout(&cust_cmd_fast_done,
		msecs_to_jiffies(2000));
	if (!timeout) {
		pr_err("%s timeout! sensor_type:%d command:%d\n", __func__, sensor_type, cust_cmd->command);
		return -ETIMEDOUT;
	}
...
EXPORT_SYMBOL_GPL(custom_cmd_comm_with);
