...
static bool transceiver_wakeup_check(uint8_t action, uint8_t sensor_type)
{
	...
	if (action == DATA_ACTION && (sensor_type == SENSOR_TYPE_PROXIMITY ||
			...
			sensor_type == COMM_SENSOR_TYPE_AMBIENT_LIGHT_SCENE ||
			sensor_type == COMM_SENSOR_TYPE_RAISEUP_DETECT ||
			sensor_type == COMM_SENSOR_TYPE_VIVOMOTION_DETECT ||
			sensor_type == COMM_SENSOR_TYPE_AMD ||
			sensor_type == SENSOR_TYPE_SAR ||
			sensor_type == SENSOR_TYPE_SAR_SECONDARY ||
			sensor_type == COMM_SENSOR_TYPE_DROP_DEPTH ||
			sensor_type == COMM_SENSOR_TYPE_SMARTPROX_DETECT))
		return true;
		...
		uint32_t write_position)
{
	...
	while (1) {
		...
		for (i = 0; i < (ret / item_size); i++) {
			memset(&evt, 0, sizeof(struct hf_manager_event));	//vivo add.ref:[B220616-2931]--10964461.220623
