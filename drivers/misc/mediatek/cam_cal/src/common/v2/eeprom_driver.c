				...
				unsigned int offset, unsigned int size)
{
	...
	//vivo lishuo modify for otp remake 20210823 [B210917-2644] begin
	...
	if (pdata->pdrv->slave_addr == 0) {
		...
		error_log("size_limit:%x!!\n", size_limit);
	}

	if (offset + size > size_limit) {
		error_log("Not support address >= 0x%x, offset:%d size:%d!!\n", size_limit,offset,size);
		return 0;
	}
	...
	if (plist && plist->readCamCalData) {
		...
		ret = Common_read_region(pdata->pdrv->pi2c_client, offset, buf, size);
		pdata->pdrv->pi2c_client->addr = dts_addr;
		mutex_unlock(&pdata->pdrv->eeprom_mutex);
	}
	must_log("read_region i2c slave addr %02x %d\n",
		 pdata->pdrv->slave_addr, ret);
	//vivo lishuo modify for otp remake 20210823 [B210917-2644] end
	return ret;
}
