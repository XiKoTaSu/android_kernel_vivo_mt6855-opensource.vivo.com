...
#define PFX "GC02M1BPD2278_camera_sensor"
#define LOG_1 LOG_INF("GC02M1BPD2278,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
char sn_inf_main2_gc02m1bpd2278[MAX_SN_SIZE + 1];
char material_inf_main2_gc02m1bpd2278[MAX_MATERIAL_SIZE + 1];
char fuse_id_main2_gc02m1bpd2278[MAX_FUSEID_SIZE + 1];
char module_tag_main2_gc02m1bpd2278[MAX_Module_SIZE + 1];

struct IMGSENSOR_SENSOR_TABLEINFO gc02m1bpd2278_sensortableinfo={
	.sensorOtpInfo = {
	...
	.sensorRawInfo = {
		...
		.raw_bitWidth = 10,
		.raw_maxSensorGain = 10 //sensor gain
	}
};

static struct imgsensor_info_struct imgsensor_info = {
	/*.sensor_id = GC02M1BV1_SENSOR_ID, */
	.sensor_id = GC02M1BPD2278_SENSOR_ID,
	...
	.custom1 = {
		.pclk = 84500000,	//record different mode's pclk
		.linelength = 2192,	//record different mode's linelength
		.framelength = 1604,	//record different mode's framelength
		.startx = 0,	//record different mode's startx of grabwindow
		.starty = 0,	//record different mode's starty of grabwindow
		.grabwindow_width = 1280,	//record different mode's width of grabwindow //1296
		.grabwindow_height = 960,	//record different mode's height of grabwindow //972
		/*     following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario    */
		.mipi_data_lp2hs_settle_dc = 85,	//unit , ns
		/*     following for GetDefaultFramerateByScenario()    */
		.mipi_pixel_rate = 67200000,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 160, 120, 1280, 960},	/*custom1*/
};
...
static void custom1_setting(void)
{
	...
	write_cmos_sensor(0xc0, 0x60);//1.5x
	write_cmos_sensor(0xc0, 0x00);
	write_cmos_sensor(0xc0, 0xc0);
	write_cmos_sensor(0xc0, 0x2a);
...
static kal_uint32 get_imgsensor_fuse_id(struct SENSOR_OTPINFO_STRUCT *data)
{
	int check_otp_read = MAIN2_2e2_otp_read();
	if(!check_otp_read){
		LOG_INF("Otp read fail!!");
		return 0;
	}
	strcpy(data->Material_inf, material_inf_main2_gc02m1bpd2278);
	strcpy(data->SN_inf, sn_inf_main2_gc02m1bpd2278);
	strcpy(data->Fuse_id, fuse_id_main2_gc02m1bpd2278);
	strcpy(data->Module_tag, module_tag_main2_gc02m1bpd2278);
	return 0;
}

static int gc02m1bpd2278GetSensorTableInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p)
{
    if(p != NULL){
        *p = &gc02m1bpd2278_sensortableinfo;
        get_imgsensor_fuse_id(&((**p).sensorOtpInfo));
    }
    return 0;
}
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	control,
	close,
	.get_sensor_tableinfo = gc02m1bpd2278GetSensorTableInfo,
};


UINT32 GC02M1BPD2278_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
