...
#define PFX "MAIN[38EE]_camera_sensor"
...
#include "s5kjnssq33pd2278mipiraw_Sensor.h"
...
#define HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjnssq03 temperature
...
struct IMGSENSOR_SENSOR_TABLEINFO s5kjnssq33pd2278_sensortableinfo={
    .sensorOtpInfo = {
        .OTP_slave_addr = 0xA0,
        .OTP_size = 0x4000,
    ...
    .sensorRawInfo = {
        ...
        .raw_height = 3060,
        .raw_dataBit = 2,
        .raw_colorFilterValue = BAYER_GRBG, //	BAYER_GRBG = 0, BAYER_GBRG = 1, BAYER_BGGR = 2, BAYER_RGGB = 3,
        .raw_blackLevel = 64,
        .raw_viewAngle = 77.1, //FOV
        .raw_bitWidth = 10,
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJNSSQ33PD2278_SENSOR_ID,
	...
	.hs_video = { /*hs_video 120fps*/
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 738400000, //1612.0
		.max_framerate = 1200,//120.06
	},
	.slim_video = { /* slim_video 120fps*/
		.pclk =598000000,
		.linelength = 2432,
		.framelength = 2048,
		.startx =0,
	...
	.custom2 = { /* 3840*2160@60fps  */
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 738400000,/*511200000*/
		.max_framerate = 600,//60.01
	},
	.custom3 = { /* 4080*3060@30fps  */
		.pclk =563333000,
		.linelength = 5392,
		.framelength = 3472,
		.startx =0,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	{8160, 6144,    0,   12, 8160, 6120, 4080, 3060, 0, 0, 4080, 3060, 0, 0, 4080, 3060}, /* custom3 */
	{8160, 6144,    0,   12, 8160, 6120, 4080, 3060, 0, 0, 4080, 3060, 0, 0, 4080, 3060}, /* custom4 */
...
{/* Preview mode setting 4080*3060*/
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 ...
	 0x01, 0x30, 0x027C, 0x0BF0, 0x00, 0x00, 0x00, 0x00},//VC DT bufW bufH
	/* Normal_Video mode setting 4080*2296*/
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x0FF0, 0x08F8, 0x00, 0x00, 0x00, 0x00,
	 0x01, 0x30, 0x027C, 0x08F0, 0x00, 0x00, 0x00, 0x00},
	/* custom2 mode setting 3840*2160 60fps*/
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x0F00, 0x0870, 0x00, 0x00, 0x00, 0x00,
	 0x01, 0x30, 0x0258, 0x0860, 0x00, 0x00, 0x00, 0x00},
	/* custom1 mode setting 3264*2448 30fps*/
	{0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,
	 0x00, 0x2b, 0x0CC0, 0x0990, 0x00, 0x00, 0x00, 0x00,
	 0x01, 0x30, 0x01FE, 0x0980, 0x00, 0x00, 0x00, 0x00},
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =
{
	...
	.i4Crop = { {0,6}, {0,6}, {0, 388}, {0, 0}, {0, 0}, {408, 312}, {120, 456}, {0, 6}, {0, 6}, {0, 6}},
};
...
static kal_uint16 addr_data_pair_custom3[] = {  // 4080x3060 30fps
	0x6028, 0x2400,
	0x602A, 0x1A28,
	0x6F12, 0x4C00,
	0x602A, 0x065A,
...
static kal_uint16 addr_data_pair_custom4[] = {  //4080x3060 30fps
	0x6028, 0x2400,
	0x602A, 0x1A28,
	0x6F12, 0x4C00,
	0x602A, 0x065A,
...
static kal_uint16 addr_data_pair_custom5[] = {  //4080x3060 30fps
	0x6028, 0x2400,
	0x602A, 0x1A28,
	0x6F12, 0x4C00,
	0x602A, 0x065A,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = read_cmos_sensor_16_16(0x0000);
			...
			if (( *sensor_id == imgsensor_info.sensor_id) || (*sensor_id == 0xe138) || (*sensor_id == 0x38ee) ) { //compatible jnv\jn1
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = read_cmos_sensor_16_16(0x0000);
			if ((sensor_id == imgsensor_info.sensor_id) || (sensor_id == 0xe138) || (sensor_id == 0x38ee)) {
	...
	if ((imgsensor_info.sensor_id != sensor_id) && (sensor_id != 0xe138) && (0x38ee != sensor_id)) {
		pr_err("Read sensor id fail, id: 0x%x\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
static kal_uint32 get_sensor_temperature(void)
{
	...
	//PK_DBG("TMC 0X000A Value=0x%4x, TMC Shift Value = 0x%4X\n", TMC_000A_Value,TMC_Shift_Value);
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 3623000;
		break;
...
/*
    ...
    */
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		switch (*feature_data) {
			case 0:
				memcpy((MUINT8 *)feature_data,(MUINT8 *)&s5kjnssq33pd2278_otpinfo,sizeof(struct IMGSENSOR_SENSOR_TABLEINFO));
				break;
			default:
...
static int s5kjnssq33pd2278GetSensorTableInfo(struct IMGSENSOR_SENSOR_TABLEINFO **p){
    if(p != NULL){
        *p = &s5kjnssq33pd2278_sensortableinfo;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	control,
	close,
	.get_sensor_tableinfo = s5kjnssq33pd2278GetSensorTableInfo, //vivo lishuo add for otp remake 20210823 [B210917-2644]
};

UINT32 S5KJNSSQ33PD2278_MIPI_RAW_SensorInit(
