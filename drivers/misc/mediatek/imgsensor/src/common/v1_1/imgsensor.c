...
static int
imgsensor_analyseOTPtype(struct IMGSENSOR_SENSOR_INST *psensor_inst) {
  struct EEPROM_DRV *pEEpInfo = psensor_inst->pEEpInfo;
  struct SENSOR_OTPINFO_STRUCT *pOtpInfo =
      &psensor_inst->psensor_tableInfo->sensorOtpInfo;
  MINT32 idx = psensor_inst->sensor_idx;
  u8 *buff, length, i, otp_cloud_flag;
  u16 sum = 0;

  pEEpInfo = find_OTP_by_idx(psensor_inst->sensor_idx);
  if (pEEpInfo == NULL || pOtpInfo == NULL) {
    PK_DBG(
        "[imgsensor_analyseOTPtype]EEpInfo/pOtpInfo is null,sensor_idx:%x\n",
        idx);
    return 0;
  } else {
  ...
  if (pOtpInfo->OTP_type == OTP_TYPE_VITRUAL) {
    return 1;
  }

  if (test_i2c_is_timeout(pOtpInfo->OTP_slave_addr)) {
    PK_DBG("[%s]eeprom i2c read 0x%02x failed\n", __func__,
           pOtpInfo->OTP_slave_addr);
    pOtpInfo->OTP_type = OTP_TYPE_EMPTY;
    if (pOtpInfo->Fuse_id[0] != '\0') {
      pOtpInfo->OTP_type = OTP_TYPE_CLOUD_NONE_EERPOM;
      return 1;
    }
  } else {
    pEEpInfo->otp_MaxAddr = pOtpInfo->OTP_size + pOtpInfo->OTP_addr_offset;
    pEEpInfo->slave_addr = pOtpInfo->OTP_slave_addr;
    if (pOtpInfo->CloudFlag_addr == 0) { // Usually,the cloudflag_addr is 0x13
      otp_read(pOtpInfo->OTP_addr_offset + OTP_CLOUD_FLAG_ADDRSHIFT, 1,
               &otp_cloud_flag, pOtpInfo->OTP_slave_addr);
    } else { // Compatible with new modules
      otp_read(pOtpInfo->OTP_addr_offset + pOtpInfo->CloudFlag_addr, 1,
               &otp_cloud_flag, pOtpInfo->OTP_slave_addr);
    }

    if (otp_cloud_flag == 1) {
      pOtpInfo->OTP_type = OTP_TYPE_CLOUD_WITH_EERPOM;
      return 1;
    } else {
      pOtpInfo->OTP_type = OTP_TYPE_NORMAL;
    }
    // vivo lishuo add for AFcode,hardcore FIX ME ,begin
    if (pOtpInfo->Af_checksum_addr > pOtpInfo->Af_addr) {
      length = pOtpInfo->Af_checksum_addr - pOtpInfo->Af_addr + 1;
      buff = kzalloc(length, GFP_KERNEL);
      if (buff != NULL) {
        otp_read(pOtpInfo->OTP_addr_offset + pOtpInfo->Af_addr, length,
                 buff, pOtpInfo->OTP_slave_addr);
        if (buff[0] == 1) { // is vaild
          ...
          if (sum == buff[length - 1]) {
            if (pOtpInfo->Af_StartCode_addr ==
                0) { // the read rule for normal module which usually hasn`t
                     // af_Mid_dis.
              pOtpInfo->Af_info[0] = buff[7];
              pOtpInfo->Af_info[1] = buff[8];
              pOtpInfo->Af_info[2] = buff[9];
              pOtpInfo->Af_info[3] = buff[10];
              pOtpInfo->Af_info[4] = buff[11];
              pOtpInfo->Af_info[5] = buff[12];
              PK_DBG("[%s]eeprom%d start\n", __func__, idx);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[0]);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[1]);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[2]);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[3]);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[4]);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[5]);
              PK_DBG("[%s]eeprom%d end\n", __func__, idx);
            } else {
              if (pOtpInfo->Af_StartCode_addr > pOtpInfo->Af_addr &&
                  pOtpInfo->Af_InfCode_addr >
                      pOtpInfo->Af_addr && // the read rule for the module
                                             // which has af_Mid_dis.
                  pOtpInfo->Af_MacroCode_addr > pOtpInfo->Af_addr) {
                pOtpInfo->Af_info[0] =
                    buff[pOtpInfo->Af_StartCode_addr - pOtpInfo->Af_addr];
                pOtpInfo->Af_info[1] = buff[pOtpInfo->Af_StartCode_addr -
                                              pOtpInfo->Af_addr + 1];
                pOtpInfo->Af_info[2] =
                    buff[pOtpInfo->Af_InfCode_addr - pOtpInfo->Af_addr];
                pOtpInfo->Af_info[3] =
                    buff[pOtpInfo->Af_InfCode_addr - pOtpInfo->Af_addr + 1];
                pOtpInfo->Af_info[4] =
                    buff[pOtpInfo->Af_MacroCode_addr - pOtpInfo->Af_addr];
                pOtpInfo->Af_info[5] = buff[pOtpInfo->Af_MacroCode_addr -
                                              pOtpInfo->Af_addr + 1];
              } else {
                PK_DBG("[%s]SPeeprom%d value illegal\n", __func__, idx);
              }
              PK_DBG("[%s]SPeeprom%d start\n", __func__, idx);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[0]);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[1]);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[2]);
              PK_DBG("[%s]%02x\n", __func__, pOtpInfo->Af_info[3]);
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
        ...
        //vivo lishuo add for otp remake 20220420 add begin
        if(psensor->pfunc->get_sensor_tableinfo != NULL){
            psensor->pfunc->get_sensor_tableinfo(&psensor_inst->psensor_tableInfo);
            imgsensor_analyseOTPtype(psensor_inst);
        }
        else{
            PK_DBG("psensor%d->pfunc->get_sensor_tableinfo is NULL\n",psensor_inst->sensor_idx);
        }
        //vivo lishuo add for otp remake 20220420 add end
		err = ERROR_NONE;
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_CLOSE:
		...
		break;
    //vivo lishuo add for otp remake 20220420 add begin
	case SENSOR_FEATURE_GET_SENSOR_TABLEINFO:{
		struct IMGSENSOR_SENSOR_TABLEINFO *pTableinfo =
			(struct IMGSENSOR_SENSOR_TABLEINFO *)pFeaturePara;
        if(pTableinfo != NULL &&  psensor->inst.psensor_tableInfo != NULL){
            memcpy((MUINT8 *)pTableinfo, (MUINT8 *)psensor->inst.psensor_tableInfo , sizeof(struct IMGSENSOR_SENSOR_TABLEINFO));
        }
		break;
    }
    //vivo lishuo add for otp remake 20220420 add end
