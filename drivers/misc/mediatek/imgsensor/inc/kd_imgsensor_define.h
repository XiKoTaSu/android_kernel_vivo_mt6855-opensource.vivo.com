...
enum ACDK_SENSOR_FEATURE_ENUM {
	...
	SENSOR_FEATURE_GET_SENSOR_TABLEINFO, //vivo lishuo add for otp remake 20220420
	SENSOR_FEATURE_MAX
};
...
struct SENSOR_OTPINFO_STRUCT {
	...
	const MUINT16 Af_disScaleRatio; //0(default) or 1 means x1, 10 means x10.only when the unit "cm" needs x10

	const MUINT16 Af_StartCode_addr; //afcode for vivo need;Don`t need to fill the addr which doesn`t exist.
	const MUINT16 Af_InfCode_addr;
	const MUINT16 Af_MidCode_addr; //reserve 20211219
	const MUINT16 Af_MacroCode_addr;

	const MUINT16 Af_InfDis_addr; //info for mtk api need;Don`t need to fill the addr which doesn`t exist.
	...
	MUINT8 Material_inf[32]; //auto fill in hal
	MUINT8 SN_inf[64]; //auto fill in hal
	MUINT8 Module_tag[20];


	const MUINT8 MtkLscType; //Usually set 2(mtk default value).
	const MUINT8 PixId; //refer to OTP guide
	const MUINT8 TableRotation; //"mirror/flip mode in OTP guide" XOR  "mirror/flip mode in sensor driver".  e.g. "H" in otp guide, "H" in sensor driver====>set 0.
...
struct IMGSENSOR_SENSOR_TABLEINFO {
	struct SENSOR_OTPINFO_STRUCT sensorOtpInfo;
	struct SENSOR_RAWINFO_STRUCT sensorRawInfo;
...
struct SENSOR_FUNCTION_STRUCT {
    ...
    int (*get_sensor_tableinfo)(struct IMGSENSOR_SENSOR_TABLEINFO **p); //vivo lishuo add for otp remake 20210823 [B210917-2644]
