...
/* add by vivo oumeiyin start */
static int set_flashlight_state(struct lm3644_flash *flash, int channel, int state)
{
	...
	case BBK_TORCH_LOW:
		...
		brt_bits = LM3644_TORCH_BRT_uA_TO_REG(fac_torch_brt * 1000);
		pr_info("%s:%d [AT+BKSG=1 or AT+BKSG=1,1] brt = %dmA(0x%x)\n", __func__, __LINE__, fac_torch_brt, brt_bits);
		if(channel == 0){
			regmap_update_bits(flash->regmap, REG_LED0_TORCH_BR, 0x7f, brt_bits);/*(Brightnees code x 1.4mA)+0.997ma Torch*/
			flash->led_mode = V4L2_FLASH_LED_MODE_TORCH;
			lm3644_mode_ctrl(flash);
			lm3644_enable_ctrl(flash, LM3644_LED0, true);
		}
		else if(channel == 1){
			regmap_update_bits(flash->regmap, REG_LED1_TORCH_BR, 0x7f, brt_bits);/*(Brightnees code x 1.4mA)+0.997ma Torch*/
			flash->led_mode = V4L2_FLASH_LED_MODE_TORCH;
			lm3644_mode_ctrl(flash);
			lm3644_enable_ctrl(flash, LM3644_LED1, true);
		}
		break;
	case BBK_TORCH_OFF:
		...
		lm3644_mode_ctrl(flash);
		if(channel == 0){
			lm3644_enable_ctrl(flash, LM3644_LED0, false);
		}
		else if(channel == 1){
			lm3644_enable_ctrl(flash, LM3644_LED1, false);
		}
	...
	case BBK_FLASH_AT_TEST:
		...
		brt_bits = LM3644_FLASH_BRT_uA_TO_REG(fac_flash_brt * 1000);
		pr_info("%s:%d [AT+FLAMP=1,1 or AT+FLAMP=2,1] brt = %dmA(0x%x)\n", __func__, __LINE__, fac_flash_brt, brt_bits);
		if(channel == 0){
			regmap_update_bits(flash->regmap, REG_LED0_FLASH_BR, 0x7f, brt_bits);/*(Brightnees code x 1.4mA)+0.997ma Torch*/
			flash->led_mode = V4L2_FLASH_LED_MODE_FLASH;
			lm3644_mode_ctrl(flash);
			lm3644_enable_ctrl(flash, LM3644_LED0, true);
		}
...
static int lm3644_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOCTL_SET_LED_STATE:
		...
		set_flashlight_state(lm3644_flash_data, channel, led_state);
		break;
	/* add by vivo oumeiyin end */
	default:
...
static int lm3644_parse_dt(struct lm3644_flash *flash)
{
	...
	for_each_child_of_node(np, cnp) {
		...
		if (of_property_read_u32(cnp,
			...
			flash->flash_dev_id[i].factory_torch_brt = LM3644_TORCH_BRT_MIN;
		}
		if (of_property_read_u32(cnp,
			...
			flash->flash_dev_id[i].factory_flash_brt = LM3644_FLASH_BRT_MIN;
		}
