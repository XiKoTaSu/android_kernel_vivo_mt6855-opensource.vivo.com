...
#define AW36515_ON              1
#define AW36515_OFF             0
#define REG_DEVICE_ID 0x0C
#define REG_FLASHIC_ID 0x00
#define AW36515_DEVICE_ID 0x02
#define AW36515_FLASHIC_ID 0x30
...
/*  FLASH Brightness
 *	min 3910uA, step 7830uA, max 2.0A in datasheet
 */
#define AW36515_FLASH_BRT_MIN 3910
#define AW36515_FLASH_BRT_STEP 7830
#define AW36515_FLASH_BRT_MAX 1200000  //lishuoyxqd protect leds
...
#define AW36515_FLASH_TOUT_MAX 320
...
static bool aw36515_judge(struct aw36515_flash *flash);
/* define usage count */
...
static int aw36515_set_power(struct aw36515_flash *flash, bool on)
{
	int ret = 0;
...
static int aw36515_write_reg(struct aw36515_flash *flash, unsigned int reg,
				     unsigned int mask, unsigned int val)
{
	int rval = -EINVAL;
	aw36515_set_power(flash, AW36515_ON);
	rval = regmap_update_bits(flash->regmap, reg, mask, val);
	return rval;
}
static int aw36515_read_reg(struct aw36515_flash *flash, unsigned int reg, unsigned int *val)
{
	int rval = -EINVAL;
	aw36515_set_power(flash, AW36515_ON);
				...
				unsigned int tout)
{
	...
	tout_bits = 0x10 + (tout / AW36515_FLASH_TOUT_STEP - 1);
...
static int aw36515_get_ctrl(struct v4l2_ctrl *ctrl, enum aw36515_led_id led_no)
{
	...
	if (ctrl->id == V4L2_CID_FLASH_FAULT) {
		...
		rval = aw36515_read_reg(flash, REG_FLAG1, &reg_val);
		if (rval < 0)
			goto out;
...
static int aw36515_set_ctrl(struct v4l2_ctrl *ctrl, enum aw36515_led_id led_no)
{
	...
	case V4L2_CID_FLASH_TIMEOUT:
		rval = aw36515_flash_tout_ctrl(flash, AW36515_FLASH_TOUT_MAX);
		break;
...
static int aw36515_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	pr_info("%s\n", __func__);
	if (!aw36515_judge(aw36515_flash_data)) {
		pr_info("%s:%d The device isn't AW36515", __func__,__LINE__);
		return -EINVAL;
	}
			      ...
			      enum aw36515_led_id led_no, char *led_name)
{
	...
	const char *fled_name = "aw36515_flash";
	int rval;
...
{
	aw36515_set_power(flash, AW36515_OFF);
...
/* add by vivo oumeiyin start */
static int set_flashlight_state(struct aw36515_flash *flash, int channel, int state)
{
	...
	case BBK_TORCH_LOW:
		...
		brt_bits = AW36515_TORCH_BRT_uA_TO_REG(fac_torch_brt * 1000);
		pr_info("%s:%d [AT+BKSG=1 or AT+BKSG=1,1] brt = %dmA(0x%x)\n", __func__, __LINE__, fac_torch_brt, brt_bits);
		if(channel == 0){
			aw36515_write_reg(flash, REG_LED0_TORCH_BR, 0xff, brt_bits); //100ma  I = val*1.96mA+0.98mA
			flash->led_mode = V4L2_FLASH_LED_MODE_TORCH;
			aw36515_mode_ctrl(flash);
			aw36515_enable_ctrl(flash, AW36515_LED0, true);
		}
		else if(channel == 1){
			aw36515_write_reg(flash, REG_LED1_TORCH_BR, 0xff, brt_bits); //100ma  I = val*1.96mA+0.98mA
			flash->led_mode = V4L2_FLASH_LED_MODE_TORCH;
			aw36515_mode_ctrl(flash);
			aw36515_enable_ctrl(flash, AW36515_LED1, true);
		}
		break;
	case BBK_TORCH_OFF:
		...
		aw36515_mode_ctrl(flash);
		if(channel == 0){
			aw36515_enable_ctrl(flash, AW36515_LED0, false);
		}
		else if(channel == 1){
			aw36515_enable_ctrl(flash, AW36515_LED1, false);
		}
		aw36515_set_power(flash, AW36515_OFF);
		break;
	case FRONT_TORCH_ON:
	...
	case BBK_FLASH_AT_TEST:
		...
		brt_bits = AW36515_FLASH_BRT_uA_TO_REG(fac_flash_brt * 1000);
		pr_info("%s:%d [AT+FLAMP=1,1 or AT+FLAMP=2,1] brt = %dmA(0x%x)\n", __func__, __LINE__, fac_flash_brt, brt_bits);
		if(channel == 0){
			aw36515_write_reg(flash, REG_LED0_FLASH_BR, 0xff, brt_bits);/*(Brightnees code x 7.83mA)+3.91ma Torch*/
			temp = AW36515_FLASH_BRT_REG_TO_uA(0x85);
			pr_info("aw36515 currrent= %d\n",temp);
			flash->led_mode = V4L2_FLASH_LED_MODE_FLASH;
			aw36515_mode_ctrl(flash);
			aw36515_enable_ctrl(flash, AW36515_LED0, true);
		}
...
static int aw36515_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_ONOFF:
		...
		if ((int)fl_arg->arg) {
			aw36515_torch_brt_ctrl(aw36515_flash_data, channel, 250000); //250mA
	...
	case FLASH_IOCTL_SET_LED_STATE:
  		...
  		set_flashlight_state(aw36515_flash_data, channel, led_state);
  		break;
	/* add by vivo oumeiyin end */
	default:
...
static int aw36515_parse_dt(struct aw36515_flash *flash)
{
	...
	for_each_child_of_node(np, cnp) {
		...
		if (of_property_read_u32(cnp,
			...
			flash->flash_dev_id[i].factory_torch_brt = AW36515_TORCH_BRT_MIN;
		}
		if (of_property_read_u32(cnp,
			...
			flash->flash_dev_id[i].factory_flash_brt = AW36515_FLASH_BRT_MIN;
		}
...
static bool aw36515_judge(struct aw36515_flash *flash)
{
	...
	//return true: The IC is AW36515; false mean the IC isn't AW36515
	//flash ic reg address: 0x0C,AW36515 value = 0x30
	//device id reg addreee: 0x02, AW36515 value = 0x02

	aw36515_read_reg(flash, REG_DEVICE_ID, &device_id_val);
	aw36515_read_reg(flash, REG_FLASHIC_ID, &flashic_id_val);
	pr_info("%s:%d DeviceID(0x%x) IC_ID(0x%x)", __func__, __LINE__, device_id_val, flashic_id_val);
	if (device_id_val == AW36515_DEVICE_ID && flashic_id_val == AW36515_FLASHIC_ID)
		return true;
	else
		return false;
}
