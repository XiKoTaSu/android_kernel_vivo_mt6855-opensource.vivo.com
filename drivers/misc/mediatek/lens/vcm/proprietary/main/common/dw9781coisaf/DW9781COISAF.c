...
static int initdrv(void)
{
	...
	ois_interface_dispatcher(OIS_DW9781C, AFIOC_X_OIS_INIT, NULL);
#else
	printk("[DW9781OISAF] ois reset start\n");
	mdelay(10);
	DW9781COISAF_I2C_OIS_per_write(0xD002, 0x0001); /*reset*/
	mdelay(4);
	DW9781COISAF_I2C_OIS_per_write(0xD001, 0x0001);/*Active mode(DSP ON)*/
	mdelay(25);/*ST gyro - over wait 25ms,default Servo on*/
	DW9781COISAF_I2C_OIS_per_write(0xEBF1, 0x56FA);/*User protection release*/
	mdelay(10);
	/*User protection release*/
	id = DW9781COISAF_I2C_OIS_per_read(0x7000);
	printk("[DW9781OISAF] ois reset end, chip id = 0x%x\n",id);

	printk("[DW9781OISAF] gyroInitOn start\n");
	mdelay(10);
	DW9781COISAF_I2C_OIS_per_write(0x7013, 0x0001); /* SPI master*/
	mdelay(55);
	DW9781COISAF_I2C_OIS_per_write(0x7017, 0x0001); /* tart To Read Gyro*/
	printk("[DW9781OISAF] gyroInitOn end\n");
