...
static unsigned int mtk_dsi_default_rate(struct mtk_dsi *dsi)
{
	...
	if ((priv->data->mmsys_id == MMSYS_MT6983 ||
		...
		//vivo yanzhenxing modify for lcm mipi change
	...
	if (status) {
		...
		if ((status & TE_RDY_INT_FLAG) && mtk_crtc &&
			...
			if (dsi->ddp_comp.id == DDP_COMPONENT_DSI0) {
				...
				vdisp_update(TYPE_TE_IRQ_HANDLE, 0, NULL);
		...
		if (status & FRAME_DONE_INT_FLAG) {
			...
			if (dsi->ddp_comp.id == DDP_COMPONENT_DSI0)
				vdisp_update(TYPE_FRAMEDONE_IRQ_HANDLE, 0, NULL);
			if (mtk_crtc && mtk_crtc->base.dev)
				priv = mtk_crtc->base.dev->dev_private;
			if (priv && mtk_drm_helper_get_opt(priv->helper_opt,
							   MTK_DRM_OPT_HBM)){
...
#ifdef CONFIG_VIVO_DISPLAY_PD2214
	...
	int force_lcm_update)
{
	...
	if (dsi->panel && dsi->ext && dsi->ext->funcs &&
		dsi->ext->funcs->lcm_pre_prepare) {
		dsi->ext->funcs->lcm_pre_prepare(dsi->panel);
	}
	...
	if (dsi->slave_dsi) {
		...
		if (ret < 0) {
			...
			if (dsi->panel && dsi->ext && dsi->ext->funcs &&
				dsi->ext->funcs->lcm_post_unprepare) {
				dsi->ext->funcs->lcm_post_unprepare(dsi->panel);
			}
			return;
		}
	}
	...
	if (dsi->panel) {
		...
		if (!mtk_crtc->res_switch && (mode_id != 0)
			&& (mtk_dsi_is_cmd_mode(&dsi->ddp_comp) ||
				(mode_chg_index & MODE_DSI_HFP) || (mode_chg_index & MODE_DSI_VFP))) {
			if (dsi->ext && dsi->ext->funcs &&
				   ...
				   int force_lcm_update)
{
	...
	if (!dsi->output_en)
		return;
	if (dsi->ddp_comp.id == DDP_COMPONENT_DSI0 && new_doze_state)
		vdisp_update(TYPE_DOZE_SUSPEND, 0, NULL);
...
void mipi_dsi_dcs_write_gce3(struct mtk_dsi *dsi, struct cmdq_pkt *dummy,
					  const void *data, size_t len)
{
	...
	if (!mtk_crtc) {
		DDPMSG("%s, mtk_crtc is null.", __func__);
		mtk_crtc = dsi->ddp_comp.mtk_crtc;
	}
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_SET_BL:
	{
		...
		#ifdef CONFIG_VIVO_LCM_SET_BL_USE_GCE3
			panel_ext->funcs->set_backlight_cmdq(dsi,
					mipi_dsi_dcs_write_gce3,
					handle, *(int *)params);
		#else
			panel_ext->funcs->set_backlight_cmdq(dsi,
