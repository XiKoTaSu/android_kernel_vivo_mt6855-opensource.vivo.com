...
#include <mtk/panel_mtk_common.h>
...
extern int mtk_disp_notifier_call_chain(unsigned long val, void *v);
...
static int mtk_drm_esd_check(struct drm_crtc *crtc)
{
	...
	/* vivo add for customized esd check begin */
	if (vdisp && !vdisp->esd.screen_down_state && esd_ctx->chk_mode == READ_LCM) {//panel_ext->params->cust_esd_check) {
		DDPINFO("Panel not face down\n");
		if (vdisp->esd.is_resume_check)
			DDPINFO("Panel resume first check power state,need alow..\n");
		else
			goto done;
	}
	/* vivo add for customized esd check end */
...
static int mtk_drm_esd_recover(struct drm_crtc *crtc)
{
	...
	if (vdisp)
		atomic_set(&vdisp->esd.recovery_pending, 1);
	...
	if (vdisp)
		atomic_set(&vdisp->esd.recovery_pending, 0);
...
/* lcm qbz add for esd recovery backlight */
extern unsigned int esd_rec_bl_level;
...
static int mtk_drm_esd_check_worker_kthread(void *data)
{
	...
	int i = 0;
	int recovery_flg = 0;
	bool need_setbacklight = false;
	int tp_suspend = LCM_NOTTIFY_TP_SUSPEND;
	int tp_resume = LCM_NOTTIFY_TP_RESUME;
	...
	while (1) {
		...
		ret = wait_event_interruptible(
			...
			(atomic_read(&esd_ctx->target_time) ||
				esd_ctx->chk_mode == READ_EINT || esd_ctx->chk_mode == READ_LCM));
		if (ret < 0) {
		...
		do {
			...
			need_setbacklight = true;
			/* add notify for tp resume and suspend */
			mtk_disp_notifier_call_chain(NOTTIFY_TP_EARLY_EVENT, &tp_suspend);
			mtk_disp_notifier_call_chain(NOTTIFY_TP_EVENT, &tp_suspend);
			mtk_disp_notifier_call_chain(NOTTIFY_TP_EARLY_EVENT, &tp_resume);
			mtk_disp_notifier_call_chain(NOTTIFY_TP_EVENT, &tp_resume);
			recovery_flg = 1;
		} while (++i < ESD_TRY_CNT);
		...
		if (need_setbacklight) {
			DDPPR_ERR("%s: need_setbacklight :%d \n", __func__, esd_rec_bl_level);
			vdisp_update(TYPE_SET_BACKLIGHT, esd_rec_bl_level, NULL);

#ifdef CONFIG_VIVO_LCM_ESD_THREE_BL_LEVEL
			vdisp_update(TYPE_SET_BACKLIGHT, 0, NULL);
			vdisp_update(TYPE_SET_BACKLIGHT, esd_rec_bl_level, NULL);
#endif
			need_setbacklight = false;
		}
