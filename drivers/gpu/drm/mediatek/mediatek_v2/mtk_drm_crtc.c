				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_HBM)) {
		...
		hbm_en = (bool)(state->prop_val[CRTC_PROP_HBM_ENABLE] & 0x1);
		/*
		mtk_drm_crtc_set_panel_hbm(crtc, hbm_en);
		mtk_drm_crtc_hbm_wait(crtc, hbm_en);
		*/
		vdisp_update_ud_flag(crtc, state->prop_val[CRTC_PROP_HBM_ENABLE]);

		if (0) //!state->prop_val[CRTC_PROP_DOZE_ACTIVE])
