...
void gpueb_dump_status(void)
{
	...
	if (g_gpueb_base) {
		...
		/* 0x13C607D0 */
		gpueb_pr_info("@%s: GPUEB_CFGREG_AXI_BIST_CON_DEBUG (0x%x): 0x%08x\n", __func__,
			(0x13C40000 + 0x207D0), readl(g_gpueb_base + 0x207D0));
		/* 0x13C607E8 */
...
static int gpueb_status_proc_show(struct seq_file *m, void *v)
{
	...
	if (g_gpueb_base) {
		...
		seq_printf(m, "GPUEB_CFGREG_MDSP_CFG (0x%x): 0x%08x\n",
			(0x13C40000 + 0x20634), readl(g_gpueb_base + 0x20634));
		/* 0x13C607D0 */
		seq_printf(m, "GPUEB_CFGREG_AXI_BIST_CON_DEBUG (0x%x): 0x%08x\n",
			(0x13C40000 + 0x207D0), readl(g_gpueb_base + 0x207D0));
	}
	if (g_gpueb_mbox_ipi)
		/* 0x13C62000 */
