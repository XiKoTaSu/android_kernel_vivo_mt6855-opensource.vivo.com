...
static struct ufs_mtk_host *ufs_fw_use_host;
...
static void ufs_mtk_trace_vh_update_sdev_vend_ss(void *data, struct scsi_device *sdev)
{
	...
	struct ufsf_feature *ufsf = ufs_mtk_get_ufsf(hba);
	struct ufs_mtk_host *mtk_host = hba->priv;
	...
	/* todo: compatible with mtk/ss */
	if (sdev->lun < ARRAY_SIZE(mtk_host->sdev_ufs_lu_ven))
		mtk_host->sdev_ufs_lu_ven[sdev->lun] = sdev;
}
...
static void ufs_mtk_trace_vh_check_int_errors(void *data, struct ufs_hba *hba,
					bool queue_eh_work)
{
...
skip_vcc:
	...
	/*
	 * By setting value of hpb_disabled as true,
	 * It can avoid activating AOSP HPB
	 */
	(container_of(hba, struct ufs_hba_add_info, hba))->hpb_dev.hpb_disabled = true;
#endif
...
struct ufs_mtk_host *ufs_mtk_get_host_ptr(void)
{
	return ufs_fw_use_host;
}
