				     ...
				     int submode)
{
	...
	dev_info(tphy->dev, "%s mode(%d), usbsel_gpio:submode(%d)\n", __func__,
		mode, submode);
	...
	if (!submode) {
		...
		pr_err("dp_dm in charger side %s, %d\n", __func__, __LINE__);
		switch (submode) {
			     ...
			     struct mtk_phy_instance *instance)
{
	...
	dev_info(dev, "host src:%d, vrt:%d, term:%d\n",
		instance->eye_src_host, instance->eye_vrt_host, instance->eye_term_host);
	dev_info(dev, "disc:%d rev4:%d, rev6:%d\n",
		instance->discth, instance->rev4, instance->rev6_host);
