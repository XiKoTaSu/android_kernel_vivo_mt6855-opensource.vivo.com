...
static int mtk_gauge_proprietary_init(struct mt6375_priv *priv)
{
	...
	pr_info("r_fg=[%d,%d],car_tune_value=%d,fg_meter_resistance=%d,com_fg_meter_resistance=%d\n",
			gauge->hw_status.r_fg_value, priv->default_r_fg, gauge->hw_status.car_tune_value,
			gauge->gm->fg_cust_data.fg_meter_resistance, gauge->gm->fg_cust_data.com_fg_meter_resistance);
	return 0;
}
