...
#define MT6375_REG_CHG_IRQ1	    0x1D1
#define MT6375_REG_CHG_IRQ2	    0x1D2
#define MT6375_REG_CHG_STAT0	0x1E0
#define MT6375_REG_CHG_STAT1	0x1E1
#define MT6375_REG_CHGDET_STAT	0x1E4
...
static const struct mt6375_chg_field mt6375_chg_fields[F_MAX] = {
	...
	MT6375_CHG_FIELD(F_CHGDET_EXT, MT6375_REG_CHGDET_STAT, 2, 2),
};
...
static void mt6375_set_dpdm_voltage(struct mt6375_chg_data *ddata, int vdp, int vdm)
{
	...
	if(1) {
		...
		if (ret < 0) {
			pr_err("failed to read usbdp voltage\n");
			goto recover;
		} else
			pr_err("DP Voltage is %d\n", dp);
		ret = iio_read_channel_processed(&ddata->iio_adcs[ADC_CHAN_USBDM], &dm);
		if (ret < 0) {
			pr_err("failed to read usbdm voltage\n");
			goto recover;
		} else
			pr_err("DM Voltage is %d\n", dm);
		return;
	}
recover:
	pr_err("set dpdm voltage error!\n");
	/* we do not guarantee the recovery is OK */
	for (; i >= 0; i--)
...
static void mt6375_get_dpdm_voltage(struct mt6375_chg_data *ddata, int *dp, int *dm)
{
	...
	if (ret < 0) {
		...
		pr_err("Get dp Vol is %d\n", *dp);

	ret = iio_read_channel_processed(&ddata->iio_adcs[ADC_CHAN_USBDM], dm);
	if (ret < 0) {
		pr_err("failed to read usbdm voltage\n");
	} else
		pr_err("Get dm Vol is %d\n", *dm);
}
#endif
...
static bool mt6375_chg_is_usb_killer(struct mt6375_chg_data *ddata)
{
	...
	vdp = U_TO_M(vdp);
	vdm = U_TO_M(vdm);
	pr_err("dp=%dmV, dm=%dmV, vdiff=%dmV\n", vdp, vdm,
	       abs(vdp - vdm));
	if (abs(vdp - vdm) < vdiff) {
		pr_err("suspect usb killer\n");
		killer = true;
	}
recover:
	/* we do not guarantee the recovery is OK */
...
static int mt6375_chg_set_cv(struct mt6375_chg_data *ddata, u32 mV)
{
	...
	if (mt6375_chg_field_set(ddata, F_CHG_EN, false) < 0)
		pr_err("failed to disable charging\n");
out_cv:
...
static void mt6375_chg_pwr_rdy_process(struct mt6375_chg_data *ddata)
{
	...
	if (ret < 0)
		pr_err("failed to set bleed discharge = %d\n",
			!ddata->pwr_rdy);
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CALIBRATE:
		...
		if (_val) {
			ret = -EBUSY;
			pr_err("vbat_mon is enabled\n");
			mutex_unlock(&ddata->cv_lock);
			break;
		}
		ret = mt6375_chg_field_set(ddata, F_VBAT_MON_EN, 1);
		if (ret < 0) {
			pr_err("failed to enable vbat monitor\n");
			mutex_unlock(&ddata->cv_lock);
			break;
		}
...
static int mt6375_run_aicc(struct charger_device *chgdev, u32 *uA)
{
	...
	if (ret < 0)
		return ret;
	if (!active) {
		pr_err("mivr loop is not active\n");
		return 0;
	}
	...
	if (ret_comp == 0)
		...
		ret = 0;
	if (ret < 0) {
		pr_err("failed to wait aicc (%d)\n", ret);
		goto out;
	}
	ret = mt6375_chg_field_get(ddata, F_AICC_RPT, uA);
	if (ret < 0) {
		pr_err("failed to get aicc report\n");
		goto out;
	}
	*uA = M_TO_U(*uA);
out:
					 ...
					 bool inc)
{
	...
	if (ret < 0) {
		pr_err("failed to set pe10 up/down\n");
		goto out;
	}
					   ...
					   u32 uV)
{
	...
	if (ret < 0) {
		pr_err("failed to set pe20 code\n", __func__);
		goto out;
	}
					    ...
					    bool en)
{
	...
	if (ret < 0) {
		pr_err("failed to set cable drop comp code\n");
		goto out;
	}
	ret = mt6375_run_pe(ddata, true);
out:
...
static int mt6375_enable_otg(struct charger_device *chgdev, bool en)
{
	...
	if (IS_ERR(regulator)) {
		pr_err("failed to get otg regulator\n");
...
static int mt6375_enable_discharge(struct charger_device *chgdev, bool en)
{
	...
	if (!en) {
		...
		if (i == dischg_retry_cnt) {
			pr_err("failed to disable dischg\n");
			ret = -EINVAL;
		}
	}
out:
...
static int mt6375_dump_registers(struct charger_device *chgdev)
{
	...
	static struct {
		...
		{ .reg = MT6375_REG_CHG_WDT, .name = "CHG_WDT" },
		{ .reg = MT6375_REG_BAT_COMP, .name = "BAT_COMP" },
		{ .reg = MT6375_REG_CHG_IRQ0, .name = "CHG_IRQ0" },
		{ .reg = MT6375_REG_CHG_IRQ1, .name = "CHG_IRQ1" },
		{ .reg = MT6375_REG_CHG_IRQ2, .name = "CHG_IRQ2" },
		{ .reg = MT6375_REG_CHGDET_STAT, .name = "CHGDET_STAT" },
	};
	struct mt6375_chg_data *ddata = charger_get_data(chgdev);
	...
	for (i = 0; i < ARRAY_SIZE(settings); i++) {
		...
		if (i == ARRAY_SIZE(settings) - 1)
			scnprintf(buf + strlen(buf), DUMP_REG_BUF_SIZE,
				  "%s = %d%s\n mt6375_dump_registers:", settings[i].name, val,
				  settings[i].unit);
		else
			scnprintf(buf + strlen(buf), DUMP_REG_BUF_SIZE,
	...
	for (i = 0; i < ARRAY_SIZE(adcs); i++) {
		...
		if (i == ARRAY_SIZE(adcs) - 1)
			scnprintf(buf + strlen(buf), DUMP_REG_BUF_SIZE,
				  "%s = %d%s\n mt6375_dump_registers:", adcs[i].name, val,
				  adcs[i].unit);
		else
			scnprintf(buf + strlen(buf), DUMP_REG_BUF_SIZE,
...
dis_pro:
	if (mt6375_chg_field_set(ddata, F_BATPROTECT_EN, 0) < 0)
		pr_err("failed to disable bat protect\n");
	if (mt6375_chg_field_set(ddata, F_CV, ddata->cv) < 0)
		pr_err("failed to set cv\n");
dis_mon:
	if (mt6375_chg_field_set(ddata, F_VBAT_MON_EN, 0) < 0)
		pr_err("failed to disable vbat monitor\n");
...
static const struct mt6375_chg_dtprop mt6375_chg_dtprops[] = {
	...
	MT6375_CHG_DTPROP(te_en, F_TE, DTPROP_BOOL, true),//enable charge current termination
...
static int mt6375_chg_apply_dt(struct mt6375_chg_data *ddata)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6375_chg_dtprops); i++) {
		...
		if (ret < 0) {
			pr_err("failed to write dtprop %s\n",
				dp->name);
			if (!dp->optional)
				return ret;
		}
...
static int mt6375_chg_get_pdata(struct device *dev)
{
	...
	if (np) {
		...
		if (!boot_np) {
			pr_err("failed to get bootmode phandle\n");
			return -ENODEV;
		}
		...
		if (!bc12_np) {
			pr_err("failed to get bc12_sel phandle\n");
			return -ENODEV;
		}
		if (of_property_read_u32(bc12_np, "bc12_sel", &val) < 0) {
			pr_err("property bc12_sel not found\n");
			return -EINVAL;
		}
		if (val != 0)
...
static int mt6375_chg_init_setting(struct mt6375_chg_data *ddata)
{
	...
	pr_err("%s\n", __func__);
	ret = mt6375_chg_field_set(ddata, F_AICC_EN, 0);
	ret = mt6375_chg_field_set(ddata, F_AICC_ONESHOT, 0);
	if (ret < 0) {
		pr_err("failed to set aicc oneshot\n");
		return ret;
	}
	...
	regmap_update_bits(ddata->rmap, MT6375_REG_CHG_TOP1, 0x10, 0x0);

	regmap_update_bits(ddata->rmap, MT6375_REG_BAT_COMP, 0x80, 0x0);//disabel bat_ircomp
	...
	if (pdata->boot_mode == 1 || pdata->boot_mode == 5) {
		...
		if (ret < 0) {
			pr_err("failed to set aicr 200mA\n");
			return ret;
		}
	}
	...
	if (ret < 0) {
		pr_err("failed to apply dt property\n");
		return ret;
	}

	ret = mt6375_chg_field_set(ddata, F_ILIM_EN, 0);
	if (ret < 0) {
		pr_err("failed to disable ilim\n");
		return ret;
	}
...
static int mt6375_chg_init_irq(struct mt6375_chg_data *ddata)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6375_chg_irqs); i++) {
		...
		if (ret < 0) {
			pr_err("failed to request irq %s\n",
				mt6375_chg_irqs[i].name);
			return ret;
		}
	}
...
static int mt6375_set_shipping_mode(struct mt6375_chg_data *ddata)
{
	...
	if (ret < 0) {
		pr_err("failed to disable ship reset\n");
		return ret;
	}

	ret = mt6375_chg_field_set(ddata, F_BATFET_DISDLY, 1);//18s
	if (ret < 0) {
		pr_err("failed to disable ship mode delay\n");
		return ret;
	}

	ret = mt6375_chg_field_set(ddata, F_BUCK_EN, 0);
	if (ret < 0) {
		pr_err("failed to disable chg buck en\n");
		return ret;
	}
...
static int mt6375_chg_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < F_MAX; i++) {
		...
		if (IS_ERR(ddata->rmap_fields[i])) {
			pr_err("failed to allocate regmap field\n");
	...
	if (ret < 0) {
		pr_err("failed to create shipping mode attribute\n");
		goto out_wq;
	}

	ret = mt6375_chg_init_setting(ddata);
	if (ret < 0) {
		pr_err("failed to init setting\n");
		goto out_attr;
	}

	ret = mt6375_chg_get_iio_adc(ddata);
	if (ret < 0) {
		pr_err("failed to get iio adc\n");
		goto out_attr;
	}
	...
	if (ret < 0) {
		pr_err("failed to init regulator\n");
		goto out_attr;
	}

	ret = mt6375_chg_init_chgdev(ddata);
	if (ret < 0) {
		pr_err("failed to init chgdev\n");
		goto out_attr;
	}
