...
static int board_ntc_get_temp(void *data, int *temp)
{
	...
	if (!r_ntc) {
		dev_err(ntc_info->dev,
			...
			adc_data->pullup_v[r_type]);
			*temp = THERMAL_TEMP_INVALID;
//#if defined(CONFIG_THERMAL_PD2214F)		
//			*temp = 30000;
//#endif			
	} else {
#if defined(CONFIG_PD2219_BOARD_TEMP)
			*temp = board_ntc_r_to_temp(ntc_info, r_ntc) - 1000;
#else
			*temp = board_ntc_r_to_temp(ntc_info, r_ntc);
#if defined(CONFIG_PD2278_BOARD_TEMP)			
			*temp = board_ntc_r_to_temp(ntc_info, r_ntc) + 3000;
#endif
#endif
	}
