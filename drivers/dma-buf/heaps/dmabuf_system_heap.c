...
extern struct ion_stats_list *stats_list;
#endif
...
#include "vivo/vivo_system_heap_alloc_trace.h"
...
#ifdef CONFIG_RMS_ION_STAT
	remove_buffer(stats_list, buffer->creator, buffer->len);
#endif
					       ...
					       const struct dma_buf_ops *ops)
{
	...
	c2 = local_clock();
	trace_system_heap_alloc(len, size_remaining, (c2 - c1) / 1000,
	...
	//20 ms timeout will print memory info
	if ((c2 - c1) / 1000 > 20000) {
		pr_info("alloc len %lu, size_remaining %lu, cost %llu us exe %llu us, "
#ifdef CONFIG_DMABUF_HEAPS_VIVO_OPT
				"NR_ION %lu, free_ion %lu, "
#endif
				"free_mem %lu",
				len, size_remaining, (c2 - c1) / 1000,
				(current->se.sum_exec_runtime - exetime) / 1000,
#ifdef CONFIG_DMABUF_HEAPS_VIVO_OPT
				get_total_dmabuf_heap_used_pages() << (PAGE_SHIFT - 10),
				get_dmabuf_free_pages() << (PAGE_SHIFT - 10),
...
#ifdef CONFIG_RMS_ION_STAT
	buffer->creator = task_tgid_nr(current);
	add_buffer(stats_list, task_tgid_nr(current), buffer->len);
#endif
