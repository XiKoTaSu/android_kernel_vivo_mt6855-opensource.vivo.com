...
struct nqx_dev {
	...
	struct timespec64 tv_read;
	struct timespec64 tv_write;
#ifdef CONFIG_NXP_NFC_RETRY
...
static void retry_wq_handler(struct work_struct *work)
{
	...
	int ret = 0;
	int retry_cnt = 0;
	if(nqx_dev->retry_info.buflen > 0) {
					...
					size_t count, loff_t *offset)
{
	...
	ktime_get_real_ts64(&nqx_dev->tv_read);
	/* Read data */
	...
	if (!nqx_dev->retry_info.is_read) {
		...
		memset(nqx_dev->retry_info.buf,0,nqx_dev->retry_info.buflen);
		nqx_dev->retry_info.buflen = 0;
		pr_err("%s:cancel_delayed_work retry_work\n", __func__);
		cancel_delayed_work_sync(&nqx_dev->retry_info.retry_work);
	}
#endif
#ifdef NFC_KERNEL_BU
				...
				size_t count, loff_t *offset)
{
	...
	ktime_get_real_ts64(&nqx_dev->tv_write);
			...
			const struct i2c_device_id *id)
{
	...
	if (nfc_support == 1) {
		if (boardversion_num == 0) {
			...
			board_version = "0";//get_board_version();
