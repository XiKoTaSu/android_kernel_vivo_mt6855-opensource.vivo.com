...
struct pinctrl_state *pins_nfc_data;
#ifdef CONFIG_OF
...
static int nfc_get_gpio_dts_info(void)
{
	...
	pins_nfc_data = pinctrl_lookup_state(pinctrl_gpios, "nfc_data");
	if (IS_ERR(pins_nfc_data)) {
		ret = PTR_ERR(pins_nfc_data);
		pr_err("%s can't find pinctrl nfc_data ret=%d\n", __func__, ret);
		return ret;
	}
	ret = pinctrl_select_state(pinctrl_gpios, pins_nfc_data);
	if (ret != 0) {
		pr_err("%s pinctrl_select_state pins_nfc_data ret =%d\n", __func__, ret);
	}
