...
static int pm8008_regulator_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
static int pm8008_chip_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int rc = 0;
	struct pm8008_chip *chip;

	pr_info("pm8008_chip_probe enter name:%s addr:%x\n", client->name, client->addr);
	...
	pr_info("PM8008 chip registered\n");
	return 0;
}
...
static struct i2c_driver pm8008_regulator_driver = {
	.driver	= {
...
static struct i2c_driver pm8008_chip_driver = {
	.driver	= {
...
static int __init pm8008_regulator_init(void)
{
	...
	rc = i2c_add_driver(&pm8008_chip_driver);
	if (rc)
		return rc;

	return i2c_add_driver(&pm8008_regulator_driver);
}
module_init(pm8008_regulator_init);

static void __exit pm8008_regulator_exit(void)
{
	i2c_del_driver(&pm8008_regulator_driver);
	i2c_del_driver(&pm8008_chip_driver);
}
